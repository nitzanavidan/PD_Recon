...
class MriModule(pl.LightningModule):
    ...
    def validation_step_end(self, val_logs):
        ...
        # modify shape to (1,320,320)
        if val_logs["output"].ndim == 2:
            val_logs["output"] = val_logs["output"].unsqueeze(0)
        elif val_logs["output"].ndim == 4:
            val_logs["output"] = val_logs["output"][:, 3, ...]
        ...
        elif val_logs["target"].ndim == 4:
            val_logs["target"] = val_logs["target"][:, 3, ...]
        ...
        elif val_logs["input"].ndim == 4:
            val_logs["input"] = val_logs["input"][:, 3, ...]
        ...
        for i, batch_idx in enumerate(batch_indices):
            if batch_idx in self.val_log_indices:
                ...
                inputs = val_logs["input"][i].unsqueeze(0)  # added
                output_kspace = val_logs["kspace_data"][i].unsqueeze(0)  # added

                error = torch.abs(target - output)
                output = output / output.max()
                target = target / target.max()
                inputs = inputs / inputs.max()  # added
                error = error / error.max()

                self.log_image(f"{key}/target", target)
                self.log_image(f"{key}/reconstruction", output)
                self.log_image(f"{key}/error", error)
                self.log_image(f"{key}/input", inputs)
                # self.log_image(f"{key}/output_kspace", output_kspace)

        # compute evaluation metrics
        mse_vals = defaultdict(dict)
        target_norms = defaultdict(dict)
        ssim_vals = defaultdict(dict)
        ssim_input = defaultdict(dict)  # added
        max_vals = dict()
        ...
        for i, fname in enumerate(val_logs["fname"]):
            ...
            inputs = val_logs["input"][i].cpu().numpy()  # added
            ...
            # add ssim of input
            ssim_input[fname][slice_num] = torch.tensor(
                evaluate.ssim(target[None, ...], inputs[None, ...], maxval=maxval)
            ).view(1)
        ...
        return {
            ...
            "ssim_input": dict(ssim_input),  # added
            "max_vals": max_vals,
        }
    ...
    def validation_epoch_end(self, val_logs):
        ...
        for val_log in val_logs:
            ...
            # add input
            for k in val_log["ssim_input"].keys():
                ssim_input[k].update(val_log["ssim_input"][k])
        ...
        assert (
                ...
                == ssim_vals.keys()
                == max_vals.keys()
                == ssim_input.keys()  # added
        )

        # apply means across image volumes
        metrics = {"nmse": 0, "ssim": 0, "psnr": 0, "ssim_input": 0}  # add "ssim_input":0
        local_examples = 0
        for fname in mse_vals.keys():
            ...
            # add ssim input
            metrics["ssim_input"] = metrics["ssim_input"] + torch.mean(
                torch.cat([v.view(-1) for _, v in ssim_input[fname].items()])
            )
