    ...
    def sample_mask(
        ...
        acceleration_mask = self.reshape_mask(
            self.calculate_acceleration_mask(
                num_cols, acceleration, offset, num_low_frequencies, idx, data_len
            ),
            shape,
        )
...
class MaskFuncRows:
    ...
    def sample_mask(
        ...
        num_rows = shape[-3]
        center_fraction, acceleration = self.choose_acceleration()
        num_low_frequencies = round(num_rows * center_fraction)
        center_mask = self.reshape_mask(
            self.calculate_center_mask(shape, num_low_frequencies), shape
        )
        acceleration_mask = self.reshape_mask(
            self.calculate_acceleration_mask(
                num_rows, acceleration, offset, num_low_frequencies, idx, data_len
            ),
            shape,
        )
...
class RandomEquiSpaceMaskFunc(MaskFunc):
    """
    Creates a random sub-sampling mask of a given shape.
    ...
    def calculate_acceleration_mask(
        ...
        # create list of
        offset_list = self.rng.randint(0, high=round(acceleration), size=data_len)
...
class EquiSpacedMaskFuncOffset1(MaskFunc):
    """
    Sample data with equally-spaced k-space lines.
...
def create_mask_for_mask_type(
    ...
    elif mask_type_str == "equispaced_offset1":
        return EquiSpacedMaskFuncOffset1(center_fractions, accelerations)
    elif mask_type_str == "equispaced_fraction":
        return EquispacedMaskFractionFunc(center_fractions, accelerations)
    elif mask_type_str == "random_equispaced":
        return RandomEquiSpaceMaskFunc(center_fractions, accelerations)
    else:
