...
class KspaceUnet(nn.Module):
    """
    PyTorch implementation of a U-Net model on kspace,data consistency term is added.
...
class KspaceResUnet(KspaceUnet):
    """
    PyTorch implementation of a U-Net model on kspace,data consistency term is added.
    O. Ronneberger, P. Fischer, and Thomas Brox. U-net: Convolutional networks
    for biomedical image segmentation. In International Conference on Medical
...
class KspaceMaskResUnet(KspaceUnet):
    """
    PyTorch implementation of a U-Net model on kspace,data consistency term is added.
    O. Ronneberger, P. Fischer, and Thomas Brox. U-net: Convolutional networks
    for biomedical image segmentation. In International Conference on Medical
    ...
    def forward(self, image: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
        ...
        if mask.shape != image.shape:
            mask_img = (torch.ones_like(image) * mask)[:, 0, ...].unsqueeze(1)
        else:
            mask_img = mask
        output = torch.cat((image, mask_img), dim=1)
